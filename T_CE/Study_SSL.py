...
sys.stdout = open('/home/hezhuangzhuang/DenoisingRec-main/T_CE/loggg.log', 'a')
...
True_pos_inter_disca = None

for epoch in range(args.epochs):
    total_discarded_users = torch.tensor([]).cuda()
    total_discarded_items = torch.tensor([]).cuda()
    total_t = torch.tensor([]).cuda()
    ...
    for user, item, label, noisy_or_not in train_loader:
        ...
        #                                                                                   loss_1 = BCE_loss(prediction_1, label)
        # loss_2 = BCE_loss(prediction_2, label)
        loss_1, discarder_user, discarder_item, t = loss_function(user,item,prediction_1, label, drop_rate_schedule(count))
        # loss_2, discarder_user, discarder_item, t = loss_function(user,item,prediction_2, label, drop_rate_schedule(count))
        # loss_1,loss_2 = loss_function_co_teaching(prediction_1, prediction_2, label, drop_rate_schedule(count))
        total_discarded_users = torch.cat((discarder_user,total_discarded_users))
        total_discarded_items = torch.cat((discarder_item,total_discarded_items))
        total_t = torch.cat((t, total_t))
        ...
        count += 1
    if True_pos_inter_disca is not None:
        user = True_pos_inter_disca[:,0].long()
        item = True_pos_inter_disca[:,1].long()
        label = torch.ones(True_pos_inter_disca.shape[0]).cuda()
        model_1.zero_grad()
        # model_2.zero_grad()
        prediction_1 = model_1(user, item)
        loss_1 = BCE_loss(prediction_1, label)
    ...
    total_discarded = torch.cat((total_discarded_users.unsqueeze(dim=1), total_discarded_items.unsqueeze(dim=1)),dim=1)
    idx = torch.where(total_t == 1)   # 找到>=3的索引
    True_pos_inter_disca = total_discarded[idx]
