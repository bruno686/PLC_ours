...
class SAT_loss():
    def __init__(self,momentum=0.9, es=2):
    ...
    def __call__(self, logits, targets):
        ...
        self.soft_labels = self.momentum * targets + (1 - self.momentum) * prob
        weights = torch.max(torch.tensor([1]).cuda() - self.soft_labels, self.soft_labels)
        weights *= logits.shape[0] / weights.sum()
        loss = torch.sum(-F.logsigmoid(logits) * self.soft_labels)
