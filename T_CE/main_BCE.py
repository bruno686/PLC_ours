...
sys.stdout = open('/home/hezhuangzhuang/DenoisingRec-main/T_CE/clean_loggg.log', 'a')
print('book clean training')
...
for epoch in range(args.epochs):
	...
	for user, item, label, noisy_or_not in train_loader:
		...
		# clean training
		indices = torch.nonzero(noisy_or_not).squeeze()
		label = label[indices]
		user = user[indices]
		item = item[indices]
  
		# label correction
		# indices = torch.where(noisy_or_not==0)
		# label[indices] = 0
